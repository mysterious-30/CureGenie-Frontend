...
import { readBarcode, getStudentProfile } from "@/services/api";
...
export default function AuthPage() {
  ...
  async function submitImage() {
    ...
    try {
      ...
      const barcodeResponse = await readBarcode(capturedImageBlob);

      if (!barcodeResponse.success || !barcodeResponse.barcode) {
        // If backend fails, try mock data as fallback
        console.warn("Backend barcode reading failed, using mock data:", barcodeResponse.message);

        // Simulate network delay for demo
        await new Promise(resolve => setTimeout(resolve, 1500));

        // Mock Success Response (fallback)
      ...
      // Backend succeeded - fetch full profile
      const studentUid = barcodeResponse.barcode;
      const firstName = barcodeResponse.firstName || "Student";

      // Fetch complete student profile
      const profileResponse = await getStudentProfile(studentUid);
      ...
      if (profileResponse && profileResponse.success) {
        ...
        handleVerificationSuccess(
          ...
          profileResponse.firstName,
          {
            ...
            name: profileResponse.firstName,
            age: profileResponse.age,
            allergy: profileResponse.allergy,
            number: profileResponse.number
          }
        );
      } else {
        // Profile fetch failed, but barcode was read - use partial data
        handleVerificationSuccess(
          studentUid,
          firstName,
          {
      ...
      handleVerificationFail("An unexpected error occurred during verification. Please try again.");
    }
  }
